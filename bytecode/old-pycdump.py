...
def unpack_pyc(filename):
    ...
    src_size = struct.unpack("i", f.read(4))[0]
    code = marshal.load(f)
    f.close()
    return filename, magic, unixtime, timestamp, code, src_size
...
def show_file(filename):
    filename, magic, unixtime, timestamp, code, src_size = unpack_pyc(filename)
